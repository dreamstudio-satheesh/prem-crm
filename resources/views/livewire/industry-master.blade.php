                ...
                <div class="card-body">
                    <div style="padding-top: 10px">
                        <table class="table table-bordered mt-5">
                            ...
                            <tbody>
                                @foreach ($industry as $rsindustry)
                                <tr>
                                    <td>{{ ($industry->currentPage() - 1) * $industry->perPage() + $loop->index + 1 }}</td>
                                    <td>{{ $rsindustry->name }}</td>
                                    <td>{{ $rsindustry->description }}</td>                                   
                                    <td>
                                        <button wire:click="edit({{ $rsindustry->id }})" class="btn btn-primary btn-sm">Edit</button>
                                        <button x-data="{ unitId: {{ $rsindustry->id }} }" @click="confirmDeletion(unitId)" class="btn btn-danger btn-sm">Delete</button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>{{ $industry->links() }}</div>
                            <div class="text-right">Total: {{ $industry->total() }}</div>
                        </div>
                    </div>
                </div>
            </div>
